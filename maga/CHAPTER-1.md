...
# Глава 1. Основы и мотивация для создания открытой доверенной операционной системы для СнК на архитектуре RISC-V
 ...
 ## 1.1. Основы и актуальность
  ...
  ### 1.1.1. Общие сведения о доверенных средах исполнения
   ...
   начиная от смартфонов, до автомобилей и серверных систем,
   используют несколько операционных систем
   одновременно на одном процессоре.
   Такие вычислительные системы обычно состоит из
   ...
   управление цифровыми правами (DRM) или обработка платежей
   ...
   и защита конфиденциальных данных и кода
 ...
 ## 1.3. Актуальность и области применения
  ...
  ### 1.3.1. Стремительный рост RISC-V в коммерческой и промышленной сферах
   ...
   #### 1.3.1.1. Широкое внедрение в вазличных областях производства
   ...
   секторы демонстрирует ее универсальность
  ...
  - Обновления по воздуху (OTA): <br>
   ...
   #### 1.3.2.2. Потенциал RISC-V в защищённых сценариях применения
 ...
 ## 1.4. Сравнение с существующими архитектурами
  ...
  - Потребность в стеке безопасного ПО: <br
  ...
  ### 1.4.6. Обзор и сравнительный анализ
   ...
   |-------------------------------------------------------|--------------------------------------------------------------|---------------------------------------------------------|
   | Аппаратное разделение на защищенный и нормальный миры | Права доступа к областям памяти, конфигурируемые из M-режима | Аппаратно-определённые, изолированные миры исполнения   |
   | Уровень ОС (Защищенная ОС vs. Обычная ОС)             | Регионы физической памяти                                    | Настраиваемая (например, на уровне ОС для каждого Мира) |
   | Полноценная Защищенная ОС в Защищенном мире           | Прошивка/монитор M-режима конфигурирует регионы              | Полноценная Защищенная ОС в выделенном Мире             |
   ...
   Данный сравнительный анализ существующих
   архитектур и механизмов,
   обеспечивающих создание ДСИ,
   выявляет различные подходы
   к достижению изолированного исполнения.
   Каждый из подходов имеет свои особенности,
   влияющие на разработку Secure OS:

  - ARM TrustZone предлагает
    хорошо зарекомендовавшую себя модель
    для комплексной защищенной ОС,
    работающей параллельно с основной ОС,
    разделяя систему на защищенный и нормальный миры.
    Несмотря на эффективность,
    её развертывание привязано
    к проприетарной архитектуре
    ARM и связанным с ней моделям лицензирования.

  - Intel SGX ориентирована
    а защиту кода и данных
    конкретных приложений внутри анклавов,
    изолируя их от потенциально
    вредоносной ОС или гипервизора.
    Эта модель, обеспечивая детальную защиту
    для целевых участков приложений,
    не предназначена для размещения полноценной,
    независимой Secure OS,
    как это преследуется в настоящей работе.
    Кроме того, лежащее в основе аппаратное обеспечение
    и микрокод представляют собой значительную
    и сложную проприетарную TCB.

  - AMD SEV нацелена на защиту
    целых виртуальных машин
    путем шифрования их памяти,
    ограждая их от скомпрометированного
    или недоверенного гипервизора.
    Это ценно для облачных сред,
    но, подобно SGX, изоляция SEV,
    сфокусированная на виртуальных машинах,
    не предназначена для модели с двумя совместно функционирующими ОС
    (Защищенной и Обычной), типичной для ДСИ
    в стиле TrustZone и модели
    рассматриваемой в данной работе.

  - Механизм защиты физической памяти (PMP)
    в RISC-V является стандартным функционалом,
    обеспечивающим фундаментальный,
    аппаратно-управляемый контроль доступа
    к регионам памяти.
    PMP - это неотъемлемый строительный блок
    для любой безопасной системы на RISC-V,
    включая ДСИ, так как он позволяет изолировать память.
    Однако одного лишь PMP недостаточно
    для определения отдельных миров
    исполнения или предоставления
    комплексной архитектурной поддержки,
    необходимой для полноценной ДСИ.
    PMP в основном служит M-режиму
    для сегментации доступа к памяти
    для менее привилегированных режимов
    и не имеет встроенных механизмов
    для переключения между мирами
    или сложного управления привилегиями
    сверх стандартных режимов RISC-V.

  - Расширение World Guard для RISC-V
    специально разработано для
    удовлетворения потребности в надежной,
    аппаратно-обеспечиваемой инфраструктуре ДСИ
    на открытой архитектуре RISC-V.
    Позволяя создавать множество изолированных миров,
    оно предоставляет прямой архитектурный аналог таким концепциям,
    ARM TrustZone, и потенциально предлагает
    более децентрализованные возможности изоляции.

 ## 1.5. Сопутствующие работы

  ### 1.5.1. Академические исследования по RISC-V ДСИ

   #### 1.5.1.1. Обзор актуальных исследований по доверенным средам RISC-V

   ...

   #### 1.5.1.2. Выявленные пробелы в академических исследованиях

   Несмотря на растущий интерес к вопросам безопасности архитектуры RISC-V, в академических исследованиях наблюдается ряд пробелов, касающихся разработки и внедрения комплексных доверенных сред исполнения (ДСИ):

   Большинство академических работ не концентрируется на безопасности.
   При этом сравнительно мало внимания уделяется проектированию
   и реализации полноценных безопасных операционных систем,
   задействующих новейшие специализированные
   расширения безопасности RISC-V,
   например, WorldGuard. Как следствие,
   специфические архитектурные преимущества
   и парадигмы безопасности таких расширений
   остаются недостаточно исследованными.

   Следует отметить, что значительная часть
   современных академических работ
   в области системной инженерии
   и операционных систем по-прежнему сфокусирована
   на устоявшихся архитектурах,
   таких как Intel, AMD и ARM.
   Как следствие, платформа RISC-V,
   несмотря на её открытость и растущий потенциал,
   часто остаётся недостаточно представленной
   в фундаментальных системных исследованиях,
   включая область проектирования ДСИ,
   что выходит за рамки узконаправленных
   публикаций по безопасности.

   Преобладание концептуальных прототипов
   и частичных решений:
   Академические реализации ДСИ
   для RISC-V представляют собой
   концептуальные прототипы (PoC),
   демонстрирующие лишь отдельные механизмы
   (например, изоляцию, аттестацию),
   но не являющиеся завершёнными,
   надёжными и готовыми к практическому применению системами.
   Это ведёт к дефициту полноценных решений,
   объединяющих безопасную загрузку,
   протоколы межмирового взаимодействия,
   стандартизированные API
   и полноценное управление жизненным циклом
   доверенных приложений.

   Исследования архитектур безопасных систем
   зачастую акцентируются на анклавных моделях.
   В то же время альтернативные подходы,
   такие как микроядерные доверенные ОС,
   capability-based модель безопасности
   и специально спроектированные для интеграции
   с аппаратными механизмами вроде расширения WorldGuard
   и их эффективного использования, остаются в тени.

   Академические прототипы зачастую упрощают
   или игнорируют трудности,
   связанные с интеграцией безопасной ОС
   в полный системный стек.
   Сюда относятся как значительные инженерные усилия
   по модификации низкоуровневого ПО (firmware),
   например, OpenSBI для обеспечения
   переключения контекстов и инициализации WorldGuard,
   так и разработка надёжных, эффективных
   и практически применимых драйверов
   для взаимодействия с Linux.

Отлично, вот перевод этого раздела на русский язык с учетом ваших пожеланий:

  ### 1.5.2. Обзор проприетарных и открытых решений ДСИ

   #### 1.5.2.1. Alibaba Cloud Link TEE

   Alibaba Cloud Link TEE – это проприетарная ДСИ,
   разработанная Alibaba Group.
   Она глубоко интегрирована в экосистему Alibaba
   и в первую очередь предназначена для
   защиты коммуникаций и данных устройств,
   подключающихся к облачным сервисам Alibaba.
   Применяется в рамках платформы интернета вещей
   и решений Alibaba IoT Edge Computing,
   обеспечивая аутентификацию устройств,
   безопасную передачу данных
   и доверенные вычисления на периферии.

   #### 1.5.2.2. Apple Secure Enclave

   Apple Secure Enclave - это аппаратная ДСИ,
   реализованная в виде выделенного
   безопасного сопроцессора,
   интегрированного в системы на кристалле от Apple.
   Эта проприетарная технология является
   ключевым элементом безопасности на устройствах Apple,
   включая iPhone, iPad, Mac и Apple Watch.
   Она отвечает за защиту
   особо чувствительных данных
   (криптографические ключи,
   биометрическая информация для Face ID и Touch ID)
   и обеспечивает целостность процессов
   безопасной загрузки.
   Secure Enclave работает под управлением
   собственного безопасного микроядра
   и использует изолированную память,
   отделенную от основного процессора.

   #### 1.5.2.3. Huawei iTrustee

   Huawei iTrustee - это проприетарное решение ДСИ,
   являющееся неотъемлемой частью
   архитектуры безопасности платформы Huawei.
   Развернуто на широком спектре устройств Huawei
   (смартфоны, планшеты),
   iTrustee обеспечивает безопасную среду
   для выполнения критически важных операций.
   Оно лежит в основе таких функций,
   как безопасная загрузка,
   доверенные платежные сервисы, DRM
   и защита конфиденциальных пользовательских данных
   в рамках закрытой экосистемы.

   #### 1.5.2.4. Google Trusty TEE

   Google Trusty - это Secure OS с открытым исходным кодом,
   разработанная и поддерживаемая Google
   преимущественно для устройств
   на базе Android и Chrome OS.
   Хотя сама ОС Trusty имеет открытый исходный код,
   ее безопасность зависит от аппаратной поддержки
   механизмов изоляции и защиты памяти.
   Она предназначена для создания безопасной среды исполнения
   для TA, которые обрабатывают чувствительные задачи,
   такие как криптографические операции,
   работа с аппаратно-защищенными хранилищами ключей и DRM.
   Trusty активно используется в смартфонах
   Google Pixel и устройствах Chromebook,
   а также может быть внедрена другими производителями.

   #### 1.5.2.5. Samsung TEEgris

   Samsung TEEgris - это проприетарная реализация ДСИ,
   разработанная Samsung Electronics.
   Она служит доверенной средой для
   платформы безопасности Samsung Knox,
   развернутой на множестве устройств Samsung
   (смартфоны, планшеты, переносимые устройства,
   умные телевизоры).
   TEEgris обеспечивает аппаратную изоляцию
   для безопасного хранения данных,
   криптографической обработки
   и выполнения доверенных приложений,
   используемых в таких функциях,
   как мобильные платежи и обеспечение
   корпоративной безопасности.
   Samsung также лицензирует технологию TEEgris
   третьим сторонам, включая OEM
