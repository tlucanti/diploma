...
# Глава 3: Проектирование и реализация доверенной операционной системы
 ...
 ## 3.10. Фреймворк доверенных приложений
  ...
  ### 3.10.4. Спецификация стандартной библиотеки для работы со строками

   #### 3.10.4.1. Утилиты для работы со строками и памятью

   Стандартная библиотека предоставляет
   базовые функции для работы с памятью и строками,
   необходимые для реализации логики
   любого доверенного приложения (ДП).

   В неё входят ключевые функции,
   такие как memset для инициализации блоков памяти,
   memcpy и memmove для копирования данных,
   а также memcmp для их сравнения.

   Кроме того, доступны стандартные утилиты для обработки строк:
   strlen, strchr, strcmp и функция strtol
   для преобразования строки в число.

  ### 3.10.5. Спецификация стандартной математической библиотеки

   #### 3.10.5.1. Алгебраические и тригонометрические функции

   Математическая библиотека предоставляет ДП
   стандартный набор математических операций.
   В неё входят как основные алгебраические функции
   (sqrt, pow, log, exp, abs, floor, ceil),
   так и полный набор тригонометрических функций
   (sin, cos, tan и обратные им asin, acos, atan).
   Для удобства также определены стандартные математические константы,
   такие как pi, e, inf и nan.

   #### 3.10.5.2. Функции для комплексных чисел

   Реализована поддержка арифметики с комплексными числами.
   Набор функций для них синтаксически отражает
   API для вещественных чисел.

  ### 3.10.6. Спецификация стандартной криптографической библиотеки

   #### 3.10.6.1. Функции хеширования

   Библиотека включает реализации
   широко используемых алгоритмов криптографического хеширования
   для поддержки проверки целостности
   и других протоколов безопасности.

   Предоставляются функции sha256, sha512, md5, crc,
   которые принимают на вход буфер данных и его длину,
   а возвращают соответствующую хэш-сумму.

   #### 3.10.6.2. Шифрование, дешифрование, управление ключами и ГПСЧ

   Функциональность для симметричного шифрования,
   выработки производных ключей
   и безопасной генерации случайных чисел
   планируется к реализации в будущем.
   Планы развития включают поддержку шифров,
   таких как aes_encrypt/aes_decrypt.

   Криптографический генератор случайных
   и псевдослучайных чисел (crypto_rng)
   спроектирован с использованием
   аппаратных и программныъ источников энтропии.

  ### 3.10.7. Спецификация стандартной библиотеки контейнеров

   #### 3.10.7.1. Функции для списков и деревьев

   Ключевой принцип проектирования библиотеки контейнеров
   заключается в том, что все древовидные структуры
   являются реентерабельными и работают
   без динамического выделения памяти (zero-alloc),
   полагаясь на узлы, встроенные в пользовательские структуры данных.

   Библиотека предлагает распространённые структуры данных,
   включая односвязный список,
   а также двусвязный список с поддержкой двунаправленного обхода.

   Для более сложной организации данных
   предоставляется префиксное (radix) дерево,
   оптимизированное для поиска
   в плотных пространствах целочисленных идентификаторов,
   и два самобалансирующихся двоичных дерева поиска:
   WAVL-дерево (ослабленный вариант AVL-дерева)
   и стандартное красно-чёрное дерево,
   реализованное на основе правил раскраски узлов.

  ### 3.10.8. Спецификация стандартной библиотеки для поддержки параллелизма

   #### 3.10.8.1. Атомарные операции и примитивы синхронизации

   Для безопасного управления параллелизмом
   библиотека предоставляет исчерпывающий набор примитивов.
   Сюда входят атомарные операции,
   такие как atomic_add_fetch и atomic_or_fetch,
   а также atomic_read и atomic_write.

   Для обеспечения порядка операций с памятью предусмотрены
   фундаментальные примитивы барьеров памяти:
   smp_rb() (барьер чтения) и smp_wb() (барьер записи).

   Для синхронизации на более высоком уровне
   реализован стандартный API для мьютексов,
   спинлоков, семафоров и условных переменных
   с соответствующими функциями инициализации,
   блокировки/ожидания, разблокировки/сигнализации и уничтожения.

  ### 3.10.9. Спецификация различных вспомогательных функций

   #### 3.10.9.1. Макросы для выравнивания, побитовых операций и работы с компилятором

   Данный модуль предоставляет набор
   низкоуровневых утилит и макросов,
   важных для системного программирования.

   Он включает макросы для
